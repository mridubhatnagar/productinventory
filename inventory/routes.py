...
def add_product_movement():
    ...
    if request.method=="POST" and form.validate():
        ...
        product_movement = ProductMovement.query.filter_by(from_location_id=from_location.id).all()

        for movement_details in product_movement:
            if movement_details.product.name not in D:
                D.update({movement_details.product.name: [movement_details.quantity]})
            else:
                D[movement_details.product.name].append(movement_details.quantity)

        for product_details in D:
            total_product_quantities[product_details] = {}
            if len(D[product_details]) > 1:
                total_product_quantities[product_details] = sum(D[product_details])
            else:
                total_product_quantities[product_details] = D[product_details][0]

        if product_id.name in total_product_quantities:
            if total_product_quantities[product_id.name] < quantity:
                flash('Not enough stock available at location', 'success')
